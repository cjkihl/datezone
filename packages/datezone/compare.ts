...
import { dayOfWeek, formatToParts, startOfDay, wallTimeToUTC } from "./index.pub";
...
function getDateOptions(ts: OptionsOrTimestamp, timeZone: TimeZone): { year: number; month: number; day: number } {
...
function getTimestamp(options: OptionsOrTimestamp, timeZone: TimeZone): number {
    ...
    return wallTimeToUTC(options.year, options.month, options.day, 0, 0, 0, 0, timeZone);
}
...
export function isToday(date: OptionsOrTimestamp, timeZone: TimeZone): boolean {
    ...
    const dateStart = typeof date === "number" ? startOfDay(date, timeZone) : startOfDay(getTimestamp(date, timeZone), timeZone);
 ...
 * Performance: Timezone required for converting timestamp to current day
 */
export function isTomorrow(date: OptionsOrTimestamp, timeZone: TimeZone): boolean {
    const todayStart = getTodayStart(timeZone);
 ...
 * Performance: Timezone required for converting timestamp to current day
 */
export function isYesterday(date: OptionsOrTimestamp, timeZone: TimeZone): boolean {
    const todayStart = getTodayStart(timeZone);
 ...
 * Performance: Timezone required for DST-aware comparison with current time
 */
export function isFuture(date: OptionsOrTimestamp, timeZone: TimeZone): boolean {
    const now = Date.now();
    const dateTs = getTimestamp(date, timeZone);
    return dateTs > now;
}
 ...
 */
export function isWeekend(date: OptionsOrTimestamp, timeZone: TimeZone): boolean {
...
export function isBefore(date1: OptionsOrTimestamp, date2: OptionsOrTimestamp, timeZone: TimeZone): boolean;
/**
...
export function isBefore(date1: OptionsOrTimestamp, date2: OptionsOrTimestamp, timeZone?: TimeZone): boolean {
    ...
    if (!timeZone) {
        throw new Error("TimeZone parameter required when comparing mixed date types");
    }
...
export function isAfter(date1: OptionsOrTimestamp, date2: OptionsOrTimestamp, timeZone: TimeZone): boolean;
/**
 * Performance: Timezone only needed for date object conversion, not DST
 * Optimized: If both inputs are same type, timezone overhead is minimized
 */
...
export function isEqual(date1: OptionsOrTimestamp, date2: OptionsOrTimestamp, timeZone: TimeZone): boolean;
/**
 ...
 * Performance: Timezone only needed for timestamp-to-date conversion, not DST
 */
export function isSameDay(date1: OptionsOrTimestamp, date2: OptionsOrTimestamp, timeZone: TimeZone): boolean {
    const start1 = typeof date1 === "number" ? startOfDay(date1, timeZone) : startOfDay(getTimestamp(date1, timeZone), timeZone);
    const start2 = typeof date2 === "number" ? startOfDay(date2, timeZone) : startOfDay(getTimestamp(date2, timeZone), timeZone);
 ...
 * Performance: Timezone only needed for timestamp-to-date conversion, not DST
 */
export function isSameWeek(date1: OptionsOrTimestamp, date2: OptionsOrTimestamp, timeZone: TimeZone): boolean {
...
export function isSameMonth(date1: OptionsOrTimestamp, date2: OptionsOrTimestamp, timeZone: TimeZone): boolean;
/**
 * Performance: Optimized for same input types - no timezone conversion needed
 */
export function isSameMonth(date1: OptionsOrTimestamp, date2: OptionsOrTimestamp, timeZone?: TimeZone): boolean {
...
export function isSameYear(date1: OptionsOrTimestamp, date2: OptionsOrTimestamp, timeZone: TimeZone): boolean;
/**
 * Performance: Optimized for same input types - no timezone conversion needed
 */
export function isSameYear(date1: OptionsOrTimestamp, date2: OptionsOrTimestamp, timeZone?: TimeZone): boolean {
