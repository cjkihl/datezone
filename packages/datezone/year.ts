...
export function startOfYear(date: OptionsOrTimestamp, timeZone: TimeZone): number {
	const { year } = getOptions(date, timeZone);
	return wallTimeToUTC(year, 1, 1, 0, 0, 0, 0, timeZone);
}

export function endOfYear(date: OptionsOrTimestamp, timeZone: TimeZone): number {
	const { year } = getOptions(date, timeZone);
	return wallTimeToUTC(year, 12, 31, 23, 59, 59, 999, timeZone);
}

export function addYears(date: OptionsOrTimestamp, amount: number, timeZone: TimeZone): number {
	if (typeof date === "number") {
		const { year, month, day, hour, minute, second } = formatToParts(date, timeZone, {
			year: "numeric",
			month: "2-digit", 
			day: "2-digit",
			hour: "2-digit",
...
export function subYears(date: OptionsOrTimestamp, amount: number, timeZone: TimeZone): number {
	return addYears(date, -amount, timeZone);
}

export function getDaysInYear(date: OptionsOrTimestamp, timeZone: TimeZone): number {
