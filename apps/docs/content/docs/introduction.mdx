...
Datezone is a lightweight, high-performance date and time-zone utility library for JavaScript and TypeScript. It provides a modern, ergonomic API for parsing, formatting, comparing, and manipulating dates and times, with full support for IANA time-zones and DST.
...
JavaScript's native `Date` object has no time-zone support, and existing libraries require additional plugins like `@date-fns/tz` or `date-fns-tz` that add complexity with wrapper objects like `TZDate`.

Datezone is built from the ground up with time-zones and DST support natively integrated into every function, eliminating the need for plugins or special wrapper objects.
...
- **45,000%+ faster** for simple date arithmetic  
...
<Callout type="info">
**Why do we need to specify a time-zone when adding days?**
...
**Example:** In the `America/New_York` time-zone on March 11, 2023 at 1:00 PM EST:
...
**Summary:** The length of a day varies depending on the time-zone and DST transitions, which is why time-zone-aware day arithmetic is essential for accurate date calculations.
</Callout>
