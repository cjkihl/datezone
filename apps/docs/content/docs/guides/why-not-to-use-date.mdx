...
- There is no API in JavaScript to override the time zone globally.
</Callout>
...
- **Predictable**: Always represent the same moment in time no matter environment and calendar system 
...
The JavaScript `Date` object is fundamentally flawed and are a big cause of Date & Time related bugs in global applications. 
