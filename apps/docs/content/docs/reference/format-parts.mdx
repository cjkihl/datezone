...
const timestamp = new Date(2024, 0, 15, 14, 30, 45, 123).getTime(); // 123ms

// Extract milliseconds along with other time components
const timeWithMs = formatToParts(timestamp, 'UTC', {
  hour: '2-digit',
  minute: '2-digit',
  second: '2-digit',
  millisecond: '3-digit',
...
console.log('Time with milliseconds:', timeWithMs);
// { hour: 14, minute: 30, second: 45, millisecond: 123 }

// Build precise time string with milliseconds
const preciseTime = `${timeWithMs.hour}:${timeWithMs.minute}:${timeWithMs.second}.${timeWithMs.millisecond.toString().padStart(3, '0')}`;
console.log('Precise time:', preciseTime); // "14:30:45.123"

// Milliseconds are timezone-independent (same across all timezones)
const utcMs = formatToParts(timestamp, 'UTC', { millisecond: '3-digit' });
const tokyoMs = formatToParts(timestamp, 'Asia/Tokyo', { millisecond: '3-digit' });
const nyMs = formatToParts(timestamp, 'America/New_York', { millisecond: '3-digit' });

console.log('UTC milliseconds:', utcMs.millisecond);     // 123
console.log('Tokyo milliseconds:', tokyoMs.millisecond); // 123
console.log('NY milliseconds:', nyMs.millisecond);       // 123
```
