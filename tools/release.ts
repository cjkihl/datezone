		...
		for (const dir of dirs) {
			if (dir.isDirectory()) {
				...
				try {
					...
					console.warn(
						`⚠️  Could not read package.json for ${dir.name}:`,
						error,
					);
				}
			}
...
async function publishPackage(
	...
	try {
		...
		if (result.exitCode === 0) {
			console.log(
				`✅ Successfully ${dryRun ? "dry-run " : ""}published package at ${packagePath}`,
			);
			return true;
		}

		console.error(
			`❌ Failed to ${dryRun ? "dry-run " : ""}publish package at ${packagePath}`,
		);
		console.error(result.stderr.toString());
		return false;
	} catch (error) {
		console.error(
			`❌ Error ${dryRun ? "dry-run " : ""}publishing package at ${packagePath}:`,
			error,
		);
		return false;
	}
...
async function main() {
	...
	console.log(
		`📋 Found ${packages.length} package(s) to ${dryRun ? "dry-run" : "publish"}:`,
	);
	...
	if (failed === 0) {
		console.log(
			`🎉 Successfully ${dryRun ? "dry-run " : ""}published all ${successful} package(s)!`,
		);
	} else {
		console.error(
			`❌ ${dryRun ? "Dry-run " : ""}Published ${successful} package(s), but ${failed} failed`,
		);
		process.exit(1);
	}
}
