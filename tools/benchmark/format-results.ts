...
class BenchmarkFormatter {
	...
	private parseOutput(output: string) {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			const benchMatch = cleanLine.match(/^([a-zA-Z0-9:\s\-()]+?)\s+(\d+(?:\.\d+)?\s*[a-zµμ]+s)\/iter/);
			if (benchMatch && currentGroup) {
				const [, name, time] = benchMatch;
				
				// Extract operations per second from the time string
				const timeValue = parseFloat(time.replace(/[^\d.]/g, ''));
...
`;
	...
	private createComparisons(benchmarks: BenchmarkResult[]): ComparisonRow[] {
		...
		const operationMap = new Map<string, { datezone?: BenchmarkResult; dateFns?: BenchmarkResult }>();
		...
		for (const bench of benchmarks) {
			...
			if (bench.name.includes('datezone:')) {
				...
				operation = bench.name.replace(/.*native:\s*/, '').replace(/\s*\(.*\)/, '');
				library = 'native';
			}
	...
	private compareResults(operation: string, datezone?: BenchmarkResult, dateFns?: BenchmarkResult): ComparisonRow {
		if (!datezone && dateFns) {
			return {
				operation,
				dateFns,
	...
	private generateSummary(): string {
		...
		const stats = {
			...
			datezoneWins: allComparisons.filter(c => c.winner === 'datezone').length,
			dateFnsWins: allComparisons.filter(c => c.winner === 'date-fns').length,
			ties: allComparisons.filter(c => c.winner === 'tie').length,
			datezoneOnly: allComparisons.filter(c => c.winner === 'datezone-only').length
		};
