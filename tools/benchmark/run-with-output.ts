...
const benchmarkConfigs: Record<string, BenchmarkConfig> = {
  "cache-test.ts": {
    ...
    description: "Performance tests focused on caching behavior and memory allocation patterns in Datezone.",
    scriptPath: "cache-test.ts",
    outputPath: "reports/cache-performance.md"
  },
  "run.ts": {
    title: "Datezone Performance Benchmarks", 
    description: "Comprehensive performance benchmarks for Datezone date/time operations across various scenarios and timezones.",
    scriptPath: "run.ts",
    outputPath: "reports/performance-benchmarks.md"
  },
  "compare.ts": {
    title: "Datezone vs Date-fns Performance Comparison",
    description: "Detailed performance comparison between Datezone and Date-fns v4 across common date/time operations.",
  ...
  "comprehensive-compare.ts": {
    ...
    description: "Comprehensive performance comparison between Datezone and Date-fns including timezone-aware operations.",
    scriptPath: "comprehensive-compare.ts",
    outputPath: "reports/comprehensive-comparison.md"
  }
};

function generateMarkdownReport(config: BenchmarkConfig, output: string): string {
  ...
  const systemInfoLines = lines.filter(line => 
    line.includes('clk:') || 
    line.includes('cpu:') || 
...
function extractBenchmarkSections(output: string): BenchmarkSection[] {
  ...
  for (const line of lines) {
    ...
    const benchmarkMatch = line.match(/^([^•\-\s].+?)\s+(\d+(?:\.\d+)?\s*[a-zA-Zμ]+\/iter)/);
    if (benchmarkMatch && currentSection) {
      ...
      if (name && time) {
        ...
        const performanceMatch = line.match(/(\d+(?:\.\d+)?x\s+(?:faster|slower))/);
...
async function runBenchmarkWithOutput(scriptName: string): Promise<void> {
  ...
  if (!config) {
    ...
    console.error(`Available scripts: ${Object.keys(benchmarkConfigs).join(', ')}`);
    process.exit(1);
  }
