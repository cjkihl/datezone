...
function parseBenchmarkOutput(output: string) {
  ...
  const benchmarkMatch = output.match(/^([^•\-\s].*?)\s+(\d+(?:\.\d+)?\s*[a-zA-Z]+\/iter).*$/);
...
function generateMarkdownReportWithOutput(title: string, description: string, capturedOutput: string): string {
  let markdown = `# ${title}\n\n`;
  markdown = `${markdown}${description}\n\n`;
  markdown = `${markdown}**Generated on:** ${new Date().toISOString()}\n\n`;
...
function generateSimpleMarkdownReport(title: string, description: string): string {
  let markdown = `# ${title}\n\n`;
  markdown = `${markdown}${description}\n\n`;
  markdown = `${markdown}**Generated on:** ${new Date().toISOString()}\n\n`;
